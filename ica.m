% Code from http://research.ics.aalto.fi/ica/imageica/ by Patrik Hoyer and Aapo Hyvärinen
% GPU support added and global variables removed

function ica_data = ica(X, whiteningMatrix, dewhiteningMatrix, p )
...
if (p.gpu)
    B = gpuArray(B);
    X = gpuArray(X);
    whiteningMatrix = gpuArray(whiteningMatrix);
    dewhiteningMatrix = gpuArray(dewhiteningMatrix);
end
...
while iter < p.iter_max  
  
  % Increment iteration counter
  iter = iter+1;
  if (mod(iter,p.write) == 0)
    fprintf('(%d) ',iter);
  end
  ...
  if strcmp(p.algorithm,'gradient')
    ...
    if rem(iter,5)==0 | iter==1
      ...
      if (rem(iter,p.write)==0), fprintf('Objective value: %.6f, Stepsize now: %.4f \n', obj(end), stepsize); end
