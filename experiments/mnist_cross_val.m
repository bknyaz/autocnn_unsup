...
opts.PCA_dim = [];
results = {};
n_filters_list = [92,192];
filter_sizes = [5:2:15];
RAs = {'0','1_3'};
file_name = 'mnist_cross-val_autocnn-s1';
n1 = 1;
for n_filters = n_filters_list
    ...
    for filter_size = filter_sizes
        n3 = 1;
        for conv_order = RAs
            opts.arch = sprintf('%dc%d-4p-conv%s', n_filters, filter_size, conv_order{1})
            r = autocnn_mnist(opts, 'conv_norm', '')
