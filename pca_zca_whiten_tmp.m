function [data, PCA_matrix, data_mean, L_regul] = pca_zca_whiten(data, labels, opts, varargin)
...
if (projection)
    ...
    for c=1:n_classes
        ...
        data_class = data(labels == labels_u(c),:);
        data_mean = mean(data_class);
        data_class = bsxfun(@minus, data_class, data_mean);
        ...
        if (opts.pca_fast)
            [evectors,evalues,~] = fsvd(data_class', dim, 2);
